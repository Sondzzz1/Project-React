            ...
            <div className="login-container" style={{ maxWidth: 480 }}>
                <div className="login-right" style={{ flex: 'none', width: '100%' }}>
                    <form className="login-form" onSubmit={handleSubmit}>
                        ...
                        <div className="form-group"><label>Email</label><input type="email" placeholder="Nhập email đã đăng ký" value={email} onChange={(e) => setEmail(e.target.value)} /></div>
                        <button type="submit" className="login-submit" disabled={loading}>{loading ? 'Đang gửi...' : 'Gửi yêu cầu'}</button>
                        <p className="login-register"><Link to="/login">← Quay lại đăng nhập</Link></p>
                    </form>
                </div>
            </div>
        </div>
