...
export default function AuditPage() {
    ...
    useEffect(() => {
        (async () => {
            try { const r = await auditApi.getLogs({ page: 1, pageSize: 50 }); setLogs(r?.data || r || []); }
                ...
                {loading ? <div className="loading-center">Đang tải...</div> : logs.length === 0 ?
                    ...
                    <table className="data-table">
                        ...
                        <tbody>{logs.map((log, i) => (
                            <tr key={i}><td>{formatDate(log.timestamp, 'full')}</td><td>{log.userName}</td><td><span className="status-badge badge-info">{log.action}</span></td><td>{log.entity}</td><td>{log.details}</td></tr>
                        ))}</tbody>
                    </table>}
            </div>
        </div>
