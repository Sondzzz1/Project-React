...
export default function BillingPage() {
    ...
    useEffect(() => {
        (async () => {
            try { const r = await billingApi.getAll(); setInvoices(r?.data || r || []); }
    ...
    const handleExportPdf = async (id) => {
        try {
            ...
            downloadBlob(blob, `hoadon_${id}.pdf`);
        } catch (e) { alert('Xuất PDF thất bại'); }
    };

    const handleExportExcel = async (id) => {
        try {
            const blob = await billingApi.exportExcel(id);
            downloadBlob(blob, `hoadon_${id}.xlsx`);
        } catch (e) { alert('Xuất Excel thất bại'); }
    };
    ...
    return (
        <div className="admin-page">
            ...
            <div className="page-toolbar">
                <div className="toolbar-left"><input className="search-input" placeholder="🔍 Tìm hóa đơn..." /></div>
            </div>
                ...
                {loading ? <div className="loading-center">Đang tải...</div> : invoices.length === 0 ?
                    ...
                    <table className="data-table">
                        ...
                        <tbody>{invoices.map(inv => (
                            <tr key={inv.id}><td>{inv.id}</td><td>{inv.tenBenhNhan}</td><td>{formatCurrency(inv.tongTien)}</td><td>{formatCurrency(inv.bhytChiTra)}</td><td>{formatCurrency(inv.conPhaiTra)}</td><td>{formatDate(inv.ngayTao)}</td>
                                <td><span className={`status-badge badge-${inv.trangThai === 'Đã thanh toán' ? 'success' : 'warning'}`}>{inv.trangThai}</span></td>
                                <td><div className="action-btns">
                                    {canExport && <><button className="btn-action btn-view" onClick={() => handleExportPdf(inv.id)}>PDF</button>
