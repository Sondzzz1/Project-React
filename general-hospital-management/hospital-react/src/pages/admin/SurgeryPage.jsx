...
export default function SurgeryPage() {
    ...
    useEffect(() => {
        (async () => {
            try { const r = await surgeryApi.getAll(); setSurgeries(r?.data || r || []); }
                ...
                {loading ? <div className="loading-center">Đang tải...</div> : surgeries.length === 0 ?
                    ...
                    <table className="data-table">
                        ...
                        <tbody>{surgeries.map(s => (
                            <tr key={s.id}><td>{s.id}</td><td>{s.tenBenhNhan}</td><td>{s.tenBacSi}</td><td>{s.loaiPhauThuat}</td><td>{formatDate(s.ngayPhauThuat)}</td>
                                <td><span className="status-badge badge-primary">{s.trangThai}</span></td></tr>
                        ))}</tbody>
                    </table>}
            </div>
        </div>
