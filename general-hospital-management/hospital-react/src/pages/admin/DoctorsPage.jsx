...
export default function DoctorsPage() {
    ...
    useEffect(() => {
        (async () => {
            try { const r = await doctorApi.getAll(); setDoctors(r?.data || r || []); }
            catch (e) { console.error(e); } finally { setLoading(false); }
        })();
    }, []);

    const filtered = doctors.filter(d => !search || (d.hoTen || '').toLowerCase().includes(search.toLowerCase()));
                ...
                {loading ? <div className="loading-center">Đang tải...</div> : filtered.length === 0 ?
                    ...
                    <table className="data-table">
                        ...
                        <tbody>{filtered.map(d => (
                            <tr key={d.id}><td>{d.id}</td><td><strong>{d.hoTen}</strong></td><td>{d.chuyenKhoa}</td><td>{d.soDienThoai || '—'}</td><td>{d.email || '—'}</td>
