...
import { bedApi, departmentApi } from '../../api';
...
export default function BedsPage() {
    ...
    const loadData = useCallback(async () => {
        try {
            ...
            setBeds(bedsRes.status === 'fulfilled' ? (bedsRes.value?.data || bedsRes.value || []) : []);
            setDepartments(deptRes.status === 'fulfilled' ? (deptRes.value?.data || deptRes.value || []) : []);
    ...
    const filteredBeds = beds.filter(b =>
        !search || (b.tenGiuong || '').toLowerCase().includes(search.toLowerCase()) ||
        (b.maGiuong || '').toLowerCase().includes(search.toLowerCase())
    );
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            else { await bedApi.create(formData); }
        ...
        <div className="admin-page">
            ...
            <div className="page-toolbar">
                <div className="toolbar-left">
                    <input className="search-input" placeholder="🔍 Tìm giường..." value={search} onChange={e => setSearch(e.target.value)} />
                </div>
                ...
                {loading ? <div className="loading-center">Đang tải...</div> : filteredBeds.length === 0 ? (
                    ...
                    <table className="data-table">
                        ...
                        <tbody>
                            {filteredBeds.map(b => (
                                <tr key={b.id}>
                                    <td><strong>{b.maGiuong}</strong></td>
                                    <td>{b.tenGiuong}</td>
                    ...
                    <div className="modal-box" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="form-row">
                            <div className="form-group"><label>Khoa</label>
                                <select value={formData.khoaId || ''} onChange={e => setFormData({ ...formData, khoaId: e.target.value })}>
